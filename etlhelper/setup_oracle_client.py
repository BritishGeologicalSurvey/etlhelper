...
class SetterUpper():
    """A class that contains attributes related to the set-up environment
    and helper methods that gather information and determine correct paths
    and variables."""

    """Install message class constants"""
    ...
    def __init__(self):
        self.client_is_configured = None
        self.url_zip_download = None
        self.zip_location = None
        self.instantclient_dir = None
        self.symlink_location = None
        self.setup_script_dir = None
        self.oracle_path_export_script_dir = None
        self.symlink_bin_dir = None
        self.oracle_path_export_symlink = None

        self.gather_setup_attributes()

    def gather_setup_attributes(self):
        """Gather the setup attributes here."""
        pass
    ...
    def setup_oracle_client(self):
        ...
        """
        if self._oracle_client_is_configured():
        ...
        if sys.platform != 'linux':
            ...
            print(self.WINDOWS_INSTALL_MESSAGE)
            sys.exit(1)

        # Create install_dir
        install_dir = self._create_install_dir()

        # Install from zipfile
        zipfile_path = self._install_zipped_files(self.zip_location, install_dir)

        # Create symlinks
        instantclient_dir = self._get_instantclient_dir(zipfile_path)
        self._create_symlinks(instantclient_dir)
        ...
        try:
            self._create_path_export_script(instantclient_dir, script_dir)
    ...
    def _create_symlinks(self):
        ...
        # Specific versions
        occi = next(self.instantclient_dir.glob('libocci.so.*.*'))
        clntsh = next(self.instantclient_dir.glob('libclntsh.so.*.*'))

        # General names
        occi_link = self.instantclient_dir / 'libocci.so'
        clntsh_link = self.instantclient_dir / 'libclntsh.so'
    ...
    def _get_instantclient_dir(self):
        """Get directory_name for instantclient based on version"""
        version = self.zipfile_path.stem.split('linux.x64-')[-1].split('.')[:2]
        dirname = '_'.join(['instantclient'] + version)
        return self.zipfile_path.parent / dirname

    def _install_zipped_files(self):
        """Unzip files from archive (downloading if required)."""
        # Check file and download if required
        if self.zip_location.lower().startswith('http'):
            self.zipfile_path = self._download_zipfile(self.zip_location, self.install_dir)
        else:
            if not os.path.isfile(self.zip_location):
                raise FileNotFoundError(self.zip_location)
            zipfile_path = self.install_dir / Path(self.zip_location).name
            if not os.path.isfile(zipfile_path):
                logging.debug(f'Copying zipfile to {zipfile_path.absolute()}')
                copyfile(self.zip_location, zipfile_path)

        # Check for previous version
        if self._get_instantclient_dir(zipfile_path).is_dir():
            logging.debug(
        ...
        # Unzip
        print(
            f'Extracting {zipfile_path} into {self.install_dir.absolute()}')
    ...
    def _download_zipfile(self):
        """Download zipfile to specified directory."""
        filename = self.zip_location.split('/')[-1]
        target = Path(self.install_dir) / filename
        if target.is_file():
            logging.debug(f'Zipfile already downloaded: {target.absolute()}')
        else:
            print(f'Downloading {self.zip_location} to {target}')
            urllib.request.urlretrieve(self.zip_location,
                                       filename=target.absolute())
        return target.absolute()

    def _oracle_client_is_configured(self):
        """
...
def main():
    ...
    setup_helper = SetterUpper(args.zip_location)
    setup_helper.setup_oracle_client()
